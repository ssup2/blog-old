...
[그림 1]은 nginx Container, http Container 및 Host의 Mount Namespace가 접근할 수 있는 Filesystem의 범위를 나타내고 있다. 기본적으로 Host는 Host Mount Namespace안에서 동작하고, Container는 Container 전용 Mount Namespace안에서 동작한다. Host Mount Namespace는 모든 Filesystem에 접근이 가능하지만 Container Mount Namespace는 일부 Mount Namespace에만 접근할 수 있다. Container Mount Namespace가 접근할 수 있는 Filesystem은 Container Root의 하위 Filesystem들 뿐이다. 따라서 Host는 각 Container의 Filesystem에 접근할 수 있지만, 각 Container는 Host의 Filesystem이나 다른 Container의 Filesystem에는 접근할 수 없다.
...
일반적으로 Docker와 같은 Container 관리 도구는 Container에게 File을 저장하는 별도의 공간인 Volume을 제공한다. 이러한 Volume은 Host의 특정 Directory를 Container의 특정 Directory에 연결하여 제공된다. 따라서 Host의 특정 File을 Container가 접근하고 싶을때, Volume을 통해서 Container에게 제공할 수 있다. 이러한 연결 과정은 Bind Mount라고 불리는 Mount 기법을 통해서 이루어진다. [Shell 1,2]을 통해서 Host에서 Volume안에 생성한 nginx_file, http_file File이 각 Container안에서도 동일하게 보이는것을 확인할 수 있다.

[Shell 1,2]에서 Volume의 Filesystem이 ext4로 되어 있고, Block Device가 /dev/sdb2인 이유는 Host의 Directory를 Bind Mout하였기 때문에 Host Root의 Filesystem과 BlockDevice가 그대로 보이는것이다. 