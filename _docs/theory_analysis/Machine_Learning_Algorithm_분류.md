...
분류를 Algorithm은 **어떤 Class(Type)**인지를 판별(예측)하는 Algorithm을 의미한다. 고양이 그림과 강아지 그림이 존재하는 상황에서 어떤 그림이 고양이이고 어떤 그림이 강아지인지 판별해야 한다면, 분류 Algorithm을 이용하면 된다. 고양이 그림, 강아지 그림 분류 Algorithm을 학습시키기 위해서는다미리 분류된 고양이 그림들과 강아지 그림들을 이용해야 한다.
