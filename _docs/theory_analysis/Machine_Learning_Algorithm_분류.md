...
분류를 수행하는 Algorithm은 의미 그대로 입력 Data가 **어떤 Class(Type)**인지 구분하는 Algorithm을 의미한다. 고양이 그림과 강아지 그림이 존재하는 상황에서 고양이 그림과 강아지 그림을 구분하는 기능이 필요한 경우 지도 학습의 분류 Algorithm을 이용하여 구현할 수 있다. 학습시 이용하는 그림들은 고양이 그림인지 강아지 그림인지 미리 분류되어 이용된다.
