...
* Select : 조건을 만족하는 튜플을 구하는 연산
* Project : 튜플중에서 특정 속성만 추출하는 연산
* Join : 두 개의 릴레이션의 공통 속성으로 합쳐서 하나의 릴레이션을 생성하는 연산
* Division : 두 개의 릴레이션에 모두 있는 속성을 가진 튜플에서 우항에 있는 속성만 구하는 연산

#### 1.9 ETC

* 참조 무결성 : 참조할 수 없는 외래키값을 가질수 없음을 의미하는 제약 조건은
