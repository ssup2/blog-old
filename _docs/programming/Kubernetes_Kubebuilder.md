...
Runtime Controller Package는 Kubernetes API Server를 통해서 Controller가 관리 해야할 CR의 변경를 감지하고, 변경된 CR의 Name과 Namespace 정보를 자신의 Work Queue에 넣는다. 그 후 Runtime Controller Package는 Work Queue에 있는 CR의 Name과 Namespace 정보를 다시 Kubebuilder Controller Package의 Reconcile Loop에 전달하여 Reconcile Loop가 동작하도록 만든다.
